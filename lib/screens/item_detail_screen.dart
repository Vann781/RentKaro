  ...
  void _handleChatPress() async {
    ...
    try {
      ...
      if (conversationId != null && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not start chat. Please try again.'), backgroundColor: Colors.red),
        );
      }
    } catch (e) {
      print("Error starting chat: $e");
          ...
          SliverList(
            delegate: SliverChildListDelegate(
              [
                ...
                // -- Owner Info --
                ListTile(
                  // --- FIX: Updated Leading Widget to use NetworkImage ---
                  leading: CircleAvatar(
                    radius: 20,
                    backgroundColor: Theme.of(context).primaryColor.withOpacity(0.8),
                    // 1. Use NetworkImage if the URL is available
                    backgroundImage: widget.item.ownerProfilePic.isNotEmpty
                        ? NetworkImage(widget.item.ownerProfilePic) as ImageProvider
                        : null,
                    // 2. Fallback to initials if the URL is empty/null
                    child: widget.item.ownerProfilePic.isEmpty
                        ? Text(
                      widget.item.ownerUsername.isNotEmpty ? widget.item.ownerUsername[0].toUpperCase() : '?',
                      style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold),
                    )
